...
def test():
    ...
    # Setup the plot: 3 rows (Rounds 1-3), 4 columns (Input, Prediction, Ground Truth, Remainder)
    # Increased figsize width to accommodate the extra column
    ...
    for round_idx in range(3):
        ...
        # Extract the Ground Truth target for this specific round
        # Assuming targets_tensor is shape [3, H, W]
        target_mask_np = targets_tensor[round_idx].cpu().numpy()
        ...
        axes[round_idx, 1].set_title(f"Round {round_idx + 1} - Extracted Prediction")
        
        axes[round_idx, 2].imshow(target_mask_np, cmap='gray')
        axes[round_idx, 2].set_title(f"Round {round_idx + 1} - Ground Truth Target")
        
        axes[round_idx, 3].imshow(img_after_subtraction, cmap='gray')
        axes[round_idx, 3].set_title(f"Round {round_idx + 1} - Image Remainder")
